#
# This function implements Jeffrey's prior for 
# finding copy number variation.
#
# Tieming, 4/23/2014
#
#
...
mvcm.jeffreys <- function(x, window.size, threshold){
# Function parameters:
...
#
  ...
  while(search.start + 1 < chrom.size){
    ...
    prod1 <- exp(2*k*(x1.bar[k]^2)-2*x1.2[k])*
             (1-pnorm(q=sqrt(1/8), mean=x1.bar[k], sd=sqrt(1/(4*k))))
    prod2 <- exp(2*(tmp.window.size-k)*(x2.bar[k+1]^2)-2*x2.2[k+1])*
